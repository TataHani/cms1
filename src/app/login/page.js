...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || 'Bledny email lub haslo')
        setLoading(false)
        return
      }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 to-slate-800">
      <div className="bg-white rounded-2xl shadow-2xl p-10 max-w-md w-full mx-4">
        ...
        <h1 className="text-2xl font-bold text-slate-900 mb-2 text-center">Logowanie</h1>
        <p className="text-slate-500 mb-8 text-center">Zaloguj sie do GMB Manager</p>
